...
func NewICETransport(gatherer *ICEGatherer) *ICETransport {
	iceTransport := &ICETransport{
		gatherer:      gatherer,
		settingEngine: gatherer.settingEngine,
		loggerFactory: gatherer.settingEngine.LoggerFactory,
		log:           gatherer.settingEngine.LoggerFactory.NewLogger("ortc"),
	}
...
func (t *ICETransport) Start(gatherer *ICEGatherer, params ICEParameters, role *ICERole) error {
	...
	config := mux.Config{
		Conn:          t.conn,
		BufferSize:    int(t.settingEngine.getReceiveMTU()),
...
func (t *ICETransport) restart() error {
	...
	if err := agent.Restart(t.settingEngine.candidates.UsernameFragment, t.settingEngine.candidates.Password); err != nil {
		return err
	}
